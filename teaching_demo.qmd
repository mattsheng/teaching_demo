...
## How do we decide? {.mid visibility="uncounted"}
...
p <- ggplot(data) +
  ...
  geom_area(data = subset(data, x > 1.96), aes(x = x, y = density_0_1), 
            fill = "red", alpha = 0.3) +
  geom_segment(aes(x = 1.96, xend = 1.96, y = 0, yend = dnorm(1.96, mean = 0, sd = 1)),
...
- [**Type II error**: Fail to reject $H_0$ given $H_0$ is false]{.fragment fragment-index=4}
...
\begin{align}
  P(\text{Type I Error})
    ...
    &\class{fragment}{{} = P(Z \geq z_\alpha } \class{fragment}{ \mid \mu = \mu_0)}         \\[3px]
    &\class{fragment}{{} = P(Z \geq z_\alpha \mid Z \sim N(0,1))}       \\[3px]
...
\begin{align}
  P(\text{Type I Error})
    ...
    &= P(Z \geq z_\alpha \mid \mu = \mu_0)            \\[3px]
    &= P(Z \geq z_\alpha \mid Z \sim N(0,1))          \\[3px]
    &= \alpha
\end{align}
$$

::: {.callout-note title="<span style='font-size: 1.75em; color: #0078D7;'>Theorem</span>"}
...
[$$z = \frac{\bar{x} - \mu_0}{\sigma/\sqrt{n}} \geq z_{\alpha}$$]{.fragment}
...
## How should we interpret Type I Error Rate?
...
<div class="fragment" style="text-align: center;">"Reject $H_0$"</div>
:::

::: {.r-stack}
![](type_I_error_1.png){.fragment .fade-in-then-out width="900" height="562"}

![](type_I_error_2.png){.fragment width="900" height="562"}
:::
...
#### Seem to be correct `r emo::ji("100")`


## How do we interpret Type II Error Rate?

::: {.callout-note title="<span style='font-size: 1.3em; color: #0078D7;'>Interpretation</span>"}
[If <span class="bold-green">$H_1: \mu = 0.3$ were true</span>, there is a 31.7\% chance that the HT procedure will lead to the conclusion of]{.fragment}

<div class="fragment" style="text-align: center;">"Fail to reject $H_0$"</div>
:::

::: {.r-stack}
![](type_I_error_1.png){.fragment .fade-in-then-out width="900" height="562"}

![](type_II_error_1.png){.fragment .fade-in-then-out width="900" height="562"}

![](type_II_error_2.png){.fragment width="900" height="562"}
:::
...
- $P(\text{Type II Error})$ or $\beta$ depends on many factors
  + $\alpha$
  + $n$ and $\sigma$
  + $\mu_1$: the actural value of $\mu$ under $H_1$
